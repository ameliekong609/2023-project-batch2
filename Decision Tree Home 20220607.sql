...
into #TEMP_quantile
 from [Deals_ASX_Analysis].[dbo].TEMP_online_trn_details2  
 where trn_date >='2021-01-01' and trn_date <='2021-12-31' 
 and (sudo_tpc in ('HO Home', 'EC Entertainment') and fulfil_node_desc  in ('Victorian Fulfilment','NSW Fulfilment','Queensland Fulfilmen', 'West Aus Fufilment') 
 or sudo_tpc in ( 'FR Furniture') )
...
---- add cumulative %
 drop table [Deals_ASX_Analysis].[dbo].TEMP_cum_vol_pct1
select *,
Sum(pct_vol) Over ( Order by Row_no ) As CumSum_BySKU


into [Deals_ASX_Analysis].[dbo].TEMP_cum_vol_pct1
from #TEMP_quantile2

select *
from [Deals_ASX_Analysis].[dbo].TEMP_cum_vol_pct1
order by Row_no
...
order by Row_no
   ...
   ---add cumulative ----
   drop table [Deals_ASX_Analysis].[dbo].DB_pareto_DarkFloor
 select *,
...
into [Deals_ASX_Analysis].[dbo].DB_pareto_DarkFloor
from #TEMP_quantile1_tpc

 select *
 from [Deals_ASX_Analysis].[dbo].TEMP_cum_vol_pct_tpc
 order by Row_no
